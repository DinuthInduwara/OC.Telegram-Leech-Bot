...
async def create_quality_menu(
    ...
    if data is None:
        ...
        for i in data.get("formats"):
            ...
            unique_formats[colity_format] = {"size": file_size, "url": url}
        ...
        for i in unique_formats.keys():
            ...
            if i == "tiny":
                ...
                text = f"{i} - [{human_readable_bytes(unique_formats[i]['url'])}] ➡️"
...
x = asyncio.run(create_quality_menu("https://www.youtube.com/watch?v=KcyN_GPG8gw", None))
print(x)
